...
export function buildAuthCookie(token, maxAgeSeconds = JWT_TTL_SECONDS) {
    const attrs = [
...
export async function requireAuth(env, request, minRole = "viewer") {
    const t = env?.__timing;
    if (t?.measure) {
...
async function requireAuthInternal(env, request, minRole) {
    ...
    if (!u || Number(u.is_active) !== 1)
        throw Object.assign(new Error("账号已禁用"), { status: 403 });
    const user = { id: u.id, username: u.username, role: u.role, must_change_password: u.must_change_password };
    ...
    try {
        ...
        if (!exp || remaining < REFRESH_THRESHOLD_SECONDS) {
            env.__refresh_token = await signJwt({ sub: user.id, u: user.username, r: user.role, tv: dbTv }, secret, JWT_TTL_SECONDS);
        }
    }
    catch { }
    return user;
}
export function json(ok, data, message, status = 200) {
