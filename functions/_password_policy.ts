export function validatePassword(raw: any) {
  const pw = String(raw ?? "");
  // Normalize by trimming leading/trailing spaces to avoid accidental lockouts.
  // (Passwords with surrounding spaces will be treated the same as without.)
  const s = pw.trim();
  if (s.length < 6) return { ok: false as const, msg: "密码至少 6 位，且必须包含字母和数字" };
  // Optional: cap length to avoid extremely long inputs
  if (s.length > 64) return { ok: false as const, msg: "密码最多 64 位" };
