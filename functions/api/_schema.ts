...
export async function ensureCoreSchema(db: D1Database) {
  const stmts: string[] = [
    ...
    "CREATE UNIQUE INDEX IF NOT EXISTS idx_item_sku_aliases_alias_active ON item_sku_aliases(alias_sku) WHERE active=1",
    "CREATE INDEX IF NOT EXISTS idx_item_sku_aliases_item_active ON item_sku_aliases(item_id, active)",
