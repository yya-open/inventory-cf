...
export const sqlStoredHoursAgo = (hours) => `datetime('now','${STORAGE_OFFSET}','-${Math.max(0, hours)} hours')`;
/** Relative timestamp in the project's persisted timezone. Example: sqlStoredDaysAgo(30). */
export const sqlStoredDaysAgo = (days) => `datetime('now','${STORAGE_OFFSET}','-${Math.max(0, days)} days')`;
/** Relative timestamp in minutes. */
export const sqlStoredMinutesAgo = (minutes) => `datetime('now','${STORAGE_OFFSET}','-${Math.max(0, minutes)} minutes')`;
/** Relative future timestamp in minutes. */
export const sqlStoredMinutesFromNow = (minutes) => `datetime('now','${STORAGE_OFFSET}','+${Math.max(0, minutes)} minutes')`;
/** SQL expr helpers for already-persisted timestamps. */
export const sqlBjDateTime = (expr) => `datetime(${expr})`;
export const sqlBjDate = (expr) => `date(${expr})`;
/** Build a Beijing-date-based filename suffix, e.g. 20260225 */
export function beijingDateStampCompact(now = new Date()) {
