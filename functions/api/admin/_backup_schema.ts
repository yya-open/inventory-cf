...
export const TABLE_DEFS: TableDef[] = [
  ...
  { name: 'audit_log', label: '审计日志', group: 'system', columns: ['id','user_id','username','action','entity','entity_id','payload_json','ip','ua','module_code','high_risk','target_name','target_code','summary_text','search_text_norm','created_at'], isSystem: true },
  { name: 'audit_retention_state', label: '审计保留策略', group: 'system', columns: ['id','retention_days','last_cleanup_at','archive_enabled','archive_after_days','delete_after_archive','max_archive_rows','warn_db_size_mb','warn_audit_rows','warn_audit_bytes_mb','last_archive_at','last_archive_before','last_archive_deleted_rows','stats_updated_at','stats_total_rows','stats_eligible_rows','stats_approx_bytes','stats_eligible_bytes','stats_oldest_at','stats_newest_at','stats_db_size_bytes'], isSystem: true },
  { name: 'audit_archive_runs', label: '审计归档记录', group: 'system', columns: ['id','job_id','archive_before','exported_rows','deleted_rows','result_object_key','result_filename','result_file_size','content_type','status','message','created_at'], isSystem: true },
  { name: 'public_api_throttle', label: '公共接口限流', group: 'system', columns: ['k','count','updated_at'], isSystem: true },
  { name: 'data_quality_cases', label: '数据质量问题', group: 'system', columns: ['id','issue_key','severity','source_table','title','detail','affected_count','sample_json','status','owner','due_at','note','first_seen_at','last_seen_at','resolved_at','updated_at'], isSystem: true },
