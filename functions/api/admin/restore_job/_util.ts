export { INTERNAL_SKIP_TABLES, sortTablesForDelete, sortTablesForInsert, pick, buildRestoreInsertSql } from '../_backup_schema';
...
export async function* iterBackupRows(file: File): AsyncGenerator<{ table: string; rowText: string }, void, unknown> {
  yield* iterBackupRowsImpl(textChunksFromFile(file));
}

export async function* iterBackupRowsFromStream(stream: ReadableStream<Uint8Array>, gzip?: boolean) {
  yield* iterBackupRowsImpl(textChunksFromStream(stream, gzip));
}

async function* iterBackupRowsImpl(chunks: AsyncIterable<string>) {
  ...
  const readJsonString = (buf: string, i: number) => {
    ...
    while (i < buf.length) {
      ...
      if (c === '"') return { value: JSON.parse(`"${out}"`), next: i };
      out += c;
    }
    return null;
  };
  ...
  for await (const chunk of chunks) {
    ...
    while (i < buf.length) {
      if (state === 'seek_tables') {
        ...
        if (idx === -1) { buf = buf.slice(Math.max(0, buf.length - 64)); i = 0; break; }
        i = idx + '"tables"'.length;
        i = skipWS(buf, i);
        if (buf[i] !== ':') { i++; continue; }
        i++; i = skipWS(buf, i);
        if (buf[i] !== '{') { i++; continue; }
        i++; state = 'seek_table_key'; continue;
      }
      ...
      if (state === 'seek_array_start') {
        ...
        if (i >= buf.length) break;
        if (buf[i] !== '[') { i++; continue; }
        i++; state = 'in_array'; objStart = -1; depth = 0; objInString = false; objEscape = false; continue;
      }
      if (state === 'in_array') {
        i = skipWS(buf, i);
        if (i >= buf.length) break;
        const c = buf[i];
        if (objStart === -1) {
          if (c === ']') { i++; state = 'seek_table_key'; continue; }
          if (c === ',') { i++; continue; }
          if (c === '{') { objStart = i; depth = 0; objInString = false; objEscape = false; } else { i++; continue; }
        }
        for (; i < buf.length; i++) {
          const ch = buf[i];
          if (objInString) {
            if (objEscape) { objEscape = false; continue; }
            if (ch === '\\') { objEscape = true; continue; }
            if (ch === '"') { objInString = false; continue; }
            continue;
          } else {
            if (ch === '"') { objInString = true; continue; }
            if (ch === '{') depth++;
            if (ch === '}') {
              depth--;
              if (depth === 0) {
        ...
        if (objStart !== -1) {
          buf = buf.slice(objStart); i = 0; objStart = 0; break;
        }
        if (i > 4096) { buf = buf.slice(i); i = 0; }
        continue;
      }
      i++;
    }
  ...
  for await (const chunk of textChunksFromStream(stream, gzip)) {
    ...
    while (i < buf.length) {
      if (state === 'seek_tables') {
        const idx = buf.indexOf('"tables"', i);
        if (idx === -1) { buf = buf.slice(Math.max(0, buf.length - 64)); i = 0; break; }
        i = idx + '"tables"'.length; i = skipWS(buf, i);
        if (buf[i] !== ':') { i++; continue; }
        i++; i = skipWS(buf, i);
        if (buf[i] !== '{') { i++; continue; }
        i++; state = 'seek_table_key'; continue;
      ...
      if (state === 'seek_table_key') {
        ...
        if (!r) break;
        const table = r.value;
        if (table && !yielded.has(table)) { yielded.add(table); yield table; }
        i = r.next; state = 'seek_array_start'; continue;
      }
      if (state === 'seek_array_start') {
        i = skipWS(buf, i);
        if (i >= buf.length) break;
        ...
        if (depth !== 0) { buf = buf.slice(Math.max(0, i - 4096)); i = 0; break; }
        state = 'seek_table_key'; continue;
      }
      i++;
    }
  }
