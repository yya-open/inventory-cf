...
export async function* textChunksFromFile(file: File, forceGzip?: boolean) {
  // NOTE: In browsers/fetch, a response with Content-Encoding:gzip may be transparently decompressed
  // while the filename still ends with .gz. So we must NOT rely on filename only.
...
async function peekStreamPrefix(stream: ReadableStream<Uint8Array>, n: number) {
  const reader = stream.getReader();
  const first = await reader.read();

  const firstChunk = (!first.done && first.value) ? first.value : new Uint8Array();
  const prefix = firstChunk.slice(0, n);
  ...
  return { prefix, stream: rebuilt };
}

export async function* textChunksFromStream(stream: ReadableStream<Uint8Array>, _gzipHint?: boolean) {
  // Auto-detect gzip by magic bytes (1F 8B). Do not rely on filename only.
  const peeked = await peekStreamPrefix(stream, 2);
  const p = peeked.prefix;
  const magicGzip = p.length >= 2 && p[0] === 0x1f && p[1] === 0x8b;

  let s: ReadableStream<Uint8Array> = peeked.stream;
  if (magicGzip && typeof (globalThis as any).DecompressionStream !== "undefined") {
    s = s.pipeThrough(new DecompressionStream("gzip"));
  }
