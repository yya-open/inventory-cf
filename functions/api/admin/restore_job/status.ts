...
export const onRequestGet: PagesFunction<{ DB: D1Database; JWT_SECRET: string }> = async ({ env, request }) => {
  try {
    ...
    return json(true, {
      ...
      backup_version: job.backup_version || null,
      integrity_status: job.integrity_status || null,
      validation: (() => { try { return JSON.parse(job.validation_json || 'null'); } catch { return null; } })(),
      verification: (() => { try { return JSON.parse(job.verification_json || 'null'); } catch { return null; } })(),
