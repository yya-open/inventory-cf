...
export const onRequestPost = async ({ env, request, waitUntil }) => {
    try {
        ...
        const warehouse_id = await assertPartsWarehouseAccess(env.DB, user, Number(body.warehouse_id ?? 1), "批量入库");
        ...
        if (!lines.length)
            return Response.json({ ok: false, message: "没有明细行" }, { status: 400 });
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!sku)
                invalid.push({ row: i + 1, reason: "sku 不能为空" });
            if (!qty || qty <= 0)
                invalid.push({ row: i + 1, reason: "qty 必须 > 0" });
        }
        if (invalid.length) {
        ...
        if (!agg.size)
            return Response.json({ ok: false, message: "有效行为空（检查 sku/qty）" }, { status: 400 });
        ...
        if (missing.length)
            return Response.json({ ok: false, message: "以下 SKU 不存在/被禁用", missing }, { status: 400 });
        const stmts = [];
        const txs = [];
        const txNos = [];
        for (const [sku, l] of agg) {
            ...
            stmts.push(env.DB.prepare(`INSERT OR IGNORE INTO stock_tx (tx_no, type, item_id, warehouse_id, qty, delta_qty, ref_type, ref_id, ref_no, unit_price, source, remark, created_by)
           VALUES (?, 'IN', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(no, item_id, warehouse_id, l.qty, l.qty, "BATCH_IN", null, ref_no, l.unit_price ?? null, l.source ?? null, l.remark ?? null, user.username));
            // 2) Only update stock if INSERT actually happened (changes()>0)
            stmts.push(env.DB.prepare(`INSERT INTO stock (item_id, warehouse_id, qty, updated_at)
           SELECT ?, ?, ?, ${sqlNowStored()}
           WHERE (SELECT changes()) > 0
           ON CONFLICT(item_id, warehouse_id) DO UPDATE SET qty = qty + excluded.qty, updated_at=${sqlNowStored()}`).bind(item_id, warehouse_id, l.qty));
        }
        ...
        waitUntil(logAudit(env.DB, request, user, "BATCH_IN", "stock_tx", client_request_id ?? batch_no, {
            warehouse_id,
            count: txs.length,
            client_request_id,
        }).catch(() => { }));
