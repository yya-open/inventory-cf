...
export const onRequestPost = async ({ env, request, waitUntil }) => {
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!tgt)
                invalid.push({ row: i + 1, reason: "target 不能为空（领用人必填）" });
        }
        if (invalid.length) {
        ...
        const { results: stockRows } = await env.DB.prepare(`SELECT item_id, qty FROM stock WHERE warehouse_id=? AND item_id IN (${ph2})`)
            ...
            .all();
        const curQty = new Map();
        for (const r of stockRows)
            curQty.set(r.item_id, Number(r.qty));
        const insufficient = [];
        for (const [sku, l] of agg) {
            const item_id = skuToId.get(sku);
            const have = curQty.get(item_id) ?? 0;
            if (have < l.qty)
                insufficient.push({ sku, need: l.qty, have });
        }
        if (insufficient.length)
            return Response.json({ ok: false, message: "库存不足", insufficient }, { status: 400 });
        ...
        for (const [sku, l] of agg) {
            ...
            stmts.push(env.DB.prepare(`INSERT OR IGNORE INTO stock_tx (tx_no, type, item_id, warehouse_id, qty, delta_qty, ref_type, ref_id, ref_no, target, remark, created_by)
           SELECT ?, 'OUT', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
           WHERE EXISTS(
             SELECT 1 FROM stock
            ...
            stmts.push(env.DB.prepare(`UPDATE stock
           SET qty = qty - ?, updated_at=${sqlNowStored()}
           WHERE item_id=? AND warehouse_id=? AND qty >= ?
             AND (SELECT changes()) > 0`).bind(l.qty, item_id, warehouse_id, l.qty));
        }
        ...
        waitUntil(logAudit(env.DB, request, user, "BATCH_OUT", "stock_tx", client_request_id ?? batch_no, {
            warehouse_id,
            count: txs.length,
            client_request_id,
        }).catch(() => { }));
