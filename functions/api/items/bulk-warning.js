...
export const onRequestPost = async ({ env, request }) => {
    try {
        ...
        const item_ids = Array.isArray(body?.item_ids) ? body.item_ids.map((x) => Number(x)).filter((n) => Number.isFinite(n)) : [];
        ...
        if (!item_ids.length)
            return Response.json({ ok: false, message: "item_ids 不能为空" }, { status: 400 });
        await assertPartsWarehouseAccess(env.DB, user, warehouse_id, '预警阈值批量设置');
        if (mode === "set") {
            if (!Number.isFinite(warning_qty) || warning_qty < 0)
                return Response.json({ ok: false, message: "warning_qty 必须是 >=0 的数字" }, { status: 400 });
        }
        else if (mode === "add") {
            if (!Number.isFinite(delta))
                return Response.json({ ok: false, message: "delta 必须是数字" }, { status: 400 });
        }
        else if (mode === "qty_plus") {
            if (!Number.isFinite(delta) || delta < 0)
                return Response.json({ ok: false, message: "delta 必须是 >=0 的数字" }, { status: 400 });
            if (!Number.isFinite(warehouse_id) || warehouse_id <= 0)
                return Response.json({ ok: false, message: "warehouse_id 不合法" }, { status: 400 });
        }
        else {
