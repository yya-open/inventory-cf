...
export const onRequestPost: PagesFunction<{ DB: D1Database; JWT_SECRET: string }> = async ({ env, request }) => {
  try {
    ...
    assertMonitorMovementAllowed(asset, 'IN');
    ...
    await logAudit(env.DB, request, user, monitorMovementAuditAction('IN'), "monitor_assets", asset.id, { tx_no, to_location_id, remark });
