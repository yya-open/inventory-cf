...
export const onRequestPost: PagesFunction<{ DB: D1Database; JWT_SECRET: string }> = async ({ env, request }) => {
  try {
    ...
    if (String(asset.status) === "SCRAPPED") throw Object.assign(new Error("该资产已报废，无法入库"), { status: 400 });

    const tx_no = monitorTxNo("MONIN");
    ...
    await logAudit(env.DB, request, user, "monitor_in", "monitor_assets", asset.id, { tx_no, to_location_id, remark });
    return Response.json({ ok: true, message: "入库成功", data: { tx_no } });
  } catch (e: any) {
