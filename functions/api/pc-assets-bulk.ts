...
export const onRequestPost = withErrorHandling<{ DB: D1Database; JWT_SECRET: string; __timing?: any }>(async ({ env, request, waitUntil }) => {
    ...
    await assertPcAssetIdsDataScopeAccess(env.DB, user, ids, '电脑批量操作');

    if (action === 'restore') {
      if (timing?.measure) await timing.measure('schema_fast', () => ensurePcReadFastGuards(env.DB));
      else await ensurePcReadFastGuards(env.DB);
    } else {
    ...
    if (action === 'restore') {
      const result = timing?.measure
        ? await timing.measure('pc_assets_restore', () => bulkRestoreAssets(env.DB, 'pc', ids))
        : await bulkRestoreAssets(env.DB, 'pc', ids);
      if (result.changed) invalidateAssetListCache('pc-assets');
      waitUntil((async () => {
        invalidateSystemDictionaryReferenceCache();
        await syncSystemDictionaryUsageCounters(env.DB, ['asset_archive_reason']);
    ...
    if (action === 'owner') {
      ...
      if (owner.department) assertDepartmentScopeAccess(user, owner.department, '电脑批量领用人更新');
      const result = await bulkUpdatePcOwner(env.DB, ids, owner, { createdBy: user.username || null });
      invalidateSystemDictionaryReferenceCache();
      if (result.changed) invalidateAssetListCache('pc-assets');
      await syncSystemDictionaryUsageCounters(env.DB, ['asset_archive_reason']);
