...
import { ensurePcSchema, optional } from './_pc';
import { createPcScrapRecord } from './services/asset-write';
...
export const onRequestPost: PagesFunction<{ DB: D1Database; JWT_SECRET: string }> = async ({ env, request, waitUntil }) => {
  try {
    ...
    const result = await createPcScrapRecord(env.DB, rows, scrap_date, reason, user.username);

    waitUntil(
      logAudit(env.DB, request, user, 'PC_SCRAP', 'pc_scrap', result.scrap_no, {
        scrap_no: result.scrap_no,
        asset_ids: assetIds,
        scrap_date,
        reason,
        count: rows.length,
    ...
    return Response.json({ ok: true, scrap_no: result.scrap_no, count: result.count });
  } catch (e: any) {
    return errorResponse(e);
  }
};
