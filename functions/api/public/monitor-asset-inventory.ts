...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    const url = new URL(request.url);
    if (timing?.measure) {
      await timing.measure('public_monitor_inventory_rate_limit', () => rateLimitPublic(env.DB, request, "public_monitor_inventory", publicAssetSubject(url), 8));
    } else {
    ...
    if (timing?.measure) {
      await timing.measure('public_monitor_inventory_insert_log', () => insertPublicInventoryLog(env.DB, "monitor", assetId, payload.action, payload.issueType, payload.remark, request));
    } else {
