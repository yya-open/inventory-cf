...
async function resolvePcAssetId(env: Env, request: Request) {
  ...
  if (idParam && keyParam) {
    ...
    if (!dbKey) throw Object.assign(new Error("该电脑尚未启用二维码"), { status: 400 });
...
// 公开盘点提交接口（扫码页按钮用）
// POST /api/public/pc-asset-inventory?id=1&key=xxxx
