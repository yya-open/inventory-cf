...
export function buildAssetQrUrl(origin: string, publicPath: AssetQrConfig['publicPath'], id: number, key: string, settings?: any) {
  const base = String(settings?.qr_public_base_url || origin || '').trim().replace(/\/$/, '');
  const strategy = String(settings?.qr_content_strategy || 'public_link').trim();
  if (strategy === 'short_query') return `${base}${publicPath}?a=${encodeURIComponent(String(id))}&k=${encodeURIComponent(key)}`;
  if (strategy === 'custom_text') return `${String(settings?.qr_custom_prefix || 'ASSET:').trim()}${publicPath.includes('monitor') ? 'MONITOR' : 'PC'}:${id}:${key}`;
  return `${base}${publicPath}?id=${encodeURIComponent(String(id))}&key=${encodeURIComponent(key)}`;
}
...
export async function getOrCreateAssetQr(db: D1Database, config: AssetQrConfig, id: number, origin: string) {
  ...
  return { id, key, url: buildAssetQrUrl(origin, config.publicPath, id, key, settings) };
}
...
export async function initMissingAssetQrKeys(db: D1Database, config: AssetQrConfig, batchSize: number) {
  ...
  const rows = await db.prepare(`SELECT id FROM ${config.assetTable} WHERE qr_key IS NULL OR TRIM(qr_key)='' ORDER BY id ASC LIMIT ?`).bind(batch).all<any>();
  const ids = (rows.results || []).map((row: any) => Number(row.id)).filter((id: number) => id > 0);
  if (!ids.length) return { updated: 0 };
  const stmts: D1PreparedStatement[] = [];
  for (const id of ids) {
    stmts.push(db.prepare(`UPDATE ${config.assetTable} SET qr_key=?, qr_updated_at=${sqlNowStored()}, updated_at=${sqlNowStored()} WHERE id=? AND (qr_key IS NULL OR TRIM(qr_key)='')`).bind(genQrKey(), id));
  }
...
export async function getOrCreateAssetQrBulk(db: D1Database, config: AssetQrConfig, ids: unknown, origin: string) {
  ...
  for (const row of result.results || []) keyMap.set(Number(row.id), String(row.qr_key || '').trim());
  const updates: D1PreparedStatement[] = [];
  for (const id of normalizedIds) {
    if (!keyMap.has(id) || keyMap.get(id)) continue;
    const key = genQrKey();
    keyMap.set(id, key);
    updates.push(db.prepare(`UPDATE ${config.assetTable} SET qr_key=?, qr_updated_at=${sqlNowStored()}, updated_at=${sqlNowStored()} WHERE id=? AND (qr_key IS NULL OR TRIM(qr_key)='')`).bind(key, id));
  }
  if (updates.length) await db.batch(updates);
  return normalizedIds
    .filter((id) => keyMap.has(id))
    .map((id) => ({ id, key: keyMap.get(id) || '', url: buildAssetQrUrl(origin, config.publicPath, id, keyMap.get(id) || '', settings) }))
