...
function isSqliteConstraintError(error: any) {
  ...
  return text.includes('SQLITE_CONSTRAINT') || text.includes('constraint failed');
}

async function findPcAssetByNormalizedSerial(db: D1Database, serialNo: string) {
  return db.prepare(`SELECT * FROM pc_assets WHERE UPPER(TRIM(serial_no))=? ORDER BY id DESC LIMIT 1`).bind(String(serialNo || '').trim().toUpperCase()).first<any>();
}

async function getPcAssetHistorySummary(db: D1Database, assetId: number) {
  const [i, o, r, s] = await Promise.all([
    db.prepare('SELECT COUNT(1) AS c FROM pc_in WHERE asset_id=?').bind(assetId).first<any>(),
    db.prepare('SELECT COUNT(1) AS c FROM pc_out WHERE asset_id=?').bind(assetId).first<any>(),
    db.prepare('SELECT COUNT(1) AS c FROM pc_recycle WHERE asset_id=?').bind(assetId).first<any>(),
    db.prepare('SELECT COUNT(1) AS c FROM pc_scrap WHERE asset_id=?').bind(assetId).first<any>(),
  ]);
  return Number(i?.c || 0) + Number(o?.c || 0) + Number(r?.c || 0) + Number(s?.c || 0);
}
...
export async function createPcAssetAndInRecord(args: CreatePcAssetArgs) {
  ...
  const normalizedSerialNo = String(serialNo || '').trim().toUpperCase();
  const manufactureTs = pcDateTextToUnixTs(manufactureDate);
  const warrantyEndTs = pcDateTextToUnixTs(warrantyEnd);
  const searchText = buildPcAssetSearchText({ brand, serial_no: normalizedSerialNo, model, remark, disk_capacity: diskCapacity, memory_size: memorySize });

  let assetId = 0;
  let insertedFreshAsset = false;
  let existing = await findPcAssetByNormalizedSerial(db, normalizedSerialNo);
  if (existing?.id) {
    const historyCount = await getPcAssetHistorySummary(db, Number(existing.id));
    if (historyCount > 0) {
    ...
    await db.prepare(
      `UPDATE pc_assets
       ...
       WHERE id=?`
    ).bind(brand, normalizedSerialNo, model, manufactureDate, warrantyEnd, manufactureTs, warrantyEndTs, diskCapacity, memorySize, remark, searchText, existing.id).run();
    ...
    try {
      ...
      ).bind(brand, normalizedSerialNo, model, manufactureDate, warrantyEnd, manufactureTs, warrantyEndTs, diskCapacity, memorySize, remark, searchText).run();
      assetId = Number((ins as any)?.meta?.last_row_id || 0) || 0;
      insertedFreshAsset = assetId > 0;
    } catch (error: any) {
      if (!isSqliteConstraintError(error)) throw error;
      existing = await findPcAssetByNormalizedSerial(db, normalizedSerialNo);
      if (!existing?.id) {
        throw Object.assign(new Error('该序列号已存在，请勿重复入库（如需入库/归还请使用「电脑回收/归还」功能）'), { status: 400 });
      }
      const historyCount = await getPcAssetHistorySummary(db, Number(existing.id));
  ...
  if (!assetId) {
    const assetRow = await findPcAssetByNormalizedSerial(db, normalizedSerialNo);
  ...
  try {
    ...
    if (insertedFreshAsset) {
      const historyCount = await getPcAssetHistorySummary(db, assetId).catch(() => 1);
      if (!historyCount) {
        await db.prepare('DELETE FROM pc_assets WHERE id=?').bind(assetId).run().catch(() => {});
      }
    }
    if (isSqliteConstraintError(error)) {
      throw Object.assign(new Error('入库单创建失败，请重试一次；若仍失败，请联系管理员检查是否存在重复提交'), { status: 400 });
    }
    throw error;
  }
