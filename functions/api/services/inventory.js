...
export async function assertItemSkuUnique(db, sku, excludeId) {
    const row = excludeId
        ? await db.prepare('SELECT id FROM items WHERE sku=? AND id<>? LIMIT 1').bind(sku, excludeId).first()
        : await db.prepare('SELECT id FROM items WHERE sku=? LIMIT 1').bind(sku).first();
    if (row?.id) {
        throw Object.assign(new Error('SKU 已存在'), { status: 400 });
    }
}
export async function createItem(db, input) {
    const result = await db.prepare(`INSERT INTO items (sku, name, brand, model, category, unit, warning_qty, created_at)
     VALUES (?,?,?,?,?,?,?, ${sqlNowStored()})`).bind(input.sku, input.name, input.brand, input.model, input.category, input.unit, input.warning_qty).run();
    const last = result?.meta?.last_row_id;
    return typeof last === 'number' ? last : (last ? Number(last) : null);
}
export async function updateItem(db, id, input) {
    await db.prepare('UPDATE items SET sku=?, name=?, brand=?, model=?, category=?, unit=?, warning_qty=? WHERE id=?').bind(input.sku, input.name, input.brand, input.model, input.category, input.unit, input.warning_qty, id).run();
}
export async function softDeleteItem(db, id) {
    await db.prepare('UPDATE items SET enabled=0 WHERE id=?').bind(id).run();
}
export async function countItems(db, query) {
    const row = await db.prepare(`SELECT COUNT(*) as c FROM items ${query.where}`).bind(...query.binds).first();
    return Number(row?.c || 0);
}
export async function listItems(db, query) {
    const result = await db.prepare(`SELECT * FROM items ${query.where} ORDER BY ${query.orderBy} LIMIT ? OFFSET ?`).bind(...query.binds, query.pageSize, query.offset).all();
    return result.results || [];
}
export function buildStockListQuery(url) {
    const keyword = normalizeKeyword(url.searchParams.get('keyword'));
...
export async function countStockRows(db, query) {
    const row = await db.prepare(`SELECT COUNT(*) as c FROM items i WHERE i.enabled=1 ${query.whereKw}`).bind(...query.binds).first();
    return Number(row?.c || 0);
}
export async function listStockRows(db, query) {
    const sql = `
    SELECT
    ...
    LIMIT ? OFFSET ?
  `;
    const result = await db.prepare(sql).bind(query.warehouse_id, ...query.binds, query.pageSize, query.offset).all();
    return result.results || [];
}
export async function getStockForItem(db, item_id, warehouse_id) {
    const row = await db.prepare(`SELECT i.id as item_id, i.warning_qty as warning_qty, COALESCE(s.qty, 0) as qty
     FROM items i
     LEFT JOIN stock s ON s.item_id = i.id AND s.warehouse_id = ?
     WHERE i.id = ? AND i.enabled = 1
     LIMIT 1`).bind(warehouse_id, item_id).first();
    return row || { item_id, warning_qty: 0, qty: 0 };
}
export function buildTxListQuery(url, options = {}) {
...
export async function countTxRows(db, query) {
    const row = await db.prepare(`SELECT COUNT(*) as c
     FROM stock_tx t
     JOIN items i ON i.id=t.item_id
     JOIN warehouses w ON w.id=t.warehouse_id
     ${query.where}`).bind(...query.binds).first();
    return Number(row?.c || 0);
}
export async function listTxRows(db, query) {
    const sql = `
...
export function buildTxExportSql(query) {
...
function buildWarningsOrder(sort) {
    switch (sort) {
        case 'gap_asc':
...
export function buildWarningsQuery(url) {
    const warehouse_id = Number(url.searchParams.get('warehouse_id') || 1);
...
export async function countWarningsRows(db, query) {
    const countSql = `SELECT COUNT(*) as c FROM ( ${buildWarningsBaseSql().replace('%WHERE%', query.whereSql)} ) x`;
    const row = await db.prepare(countSql).bind(query.warehouse_id, query.warehouse_id, ...query.binds).first();
    return Number(row?.c || 0);
}
export async function listWarningsRows(db, query) {
    const sql = `${buildWarningsBaseSql().replace('%WHERE%', query.whereSql)} ORDER BY ${query.orderBy} LIMIT ? OFFSET ?`;
    const result = await db.prepare(sql).bind(query.warehouse_id, query.warehouse_id, ...query.binds, query.pageSize, query.offset).all();
    return result.results || [];
}
export async function listWarningsExportRows(db, query) {
    const sql = `
    SELECT
    ...
    ORDER BY ${query.orderBy}
  `;
    const result = await db.prepare(sql).bind(query.warehouse_id, query.warehouse_id, query.warehouse_id, ...query.binds).all();
    return result.results || [];
}
export async function getWarehouseName(db, warehouse_id) {
    const row = await db.prepare('SELECT name FROM warehouses WHERE id=?').bind(warehouse_id).first();
