    ...
    try {
        ...
        try {
            await env.DB.batch([
                ...
                env.DB.prepare(`INSERT OR IGNORE INTO stock_tx (
             tx_no, type, item_id, warehouse_id, qty, delta_qty,
             ref_type, ref_id, ref_no, unit_price, source, remark, created_by
           ) VALUES (?, 'IN', ?, ?, ?, ?, 'MANUAL_IN', NULL, ?, ?, ?, ?, ?);`).bind(no, item_id, allowedWarehouseId, q, q, refNo || no, unit_price ?? null, source ?? null, remark ?? null, user.username),
                // 2) Stock upsert ONLY if statement (1) actually inserted a row
                env.DB.prepare(`INSERT INTO stock (item_id, warehouse_id, qty, updated_at)
           SELECT ?, ?, ?, ${sqlNowStored()}
           WHERE (SELECT changes()) > 0
           ON CONFLICT(item_id, warehouse_id)
           DO UPDATE SET qty = qty + excluded.qty, updated_at=${sqlNowStored()};`).bind(item_id, allowedWarehouseId, q),
        ...
        const insertedRow = await env.DB.prepare(`SELECT tx_no FROM stock_tx WHERE tx_no=? LIMIT 1`).bind(no).first();
        if (!insertedRow?.tx_no) {
            if (refNo) {
                const exist = await env.DB.prepare(`SELECT tx_no FROM stock_tx WHERE ref_no=? LIMIT 1`).bind(refNo).first();
                if (exist?.tx_no)
                    return Response.json({ ok: true, tx_no: exist.tx_no, duplicate: true });
            }
