...
export const onRequestPost = async ({ env, request, waitUntil }) => {
    try {
        ...
        const scopedUser = Object.assign({}, user, await getUserDataScope(env.DB, user.id).catch(() => ({ data_scope_type: 'all', data_scope_value: null, data_scope_value2: null })));
        ...
        await assertPartsStocktakeAccess(env.DB, scopedUser, st_id, '库存盘点');
        ...
        if (status === 'DRAFT') {
            ...
            if (up?.meta?.changes !== 1) {
                const cur = await env.DB.prepare(`SELECT status FROM stocktake WHERE id=?`).bind(st_id).first();
        ...
        const rows = (await env.DB.prepare(`SELECT * FROM stocktake_line WHERE stocktake_id=? AND counted_qty IS NOT NULL`).bind(st_id).all()).results || [];
        const preview = {
            counted_rows: rows.length,
            adjusted_rows: rows.filter((r) => Number(r?.diff_qty || 0) !== 0).length,
            increase_rows: rows.filter((r) => Number(r?.diff_qty || 0) > 0).length,
            decrease_rows: rows.filter((r) => Number(r?.diff_qty || 0) < 0).length,
        };
        if (previewOnly) {
        ...
        for (let i = 0; i < rows.length; i += CHUNK_LINES) {
            ...
            for (const r of part) {
                ...
                stmts.push(env.DB.prepare(`INSERT OR IGNORE INTO stock (item_id, warehouse_id, qty, updated_at) VALUES (?, ?, 0, ${sqlNowStored()})`).bind(itemId, warehouseId), env.DB.prepare(`UPDATE stock
             ...
             WHERE item_id=? AND warehouse_id=?
               ...
               )`).bind(diff, itemId, warehouseId, diff, st_id, itemId, warehouseId), env.DB.prepare(`INSERT INTO stock_tx (tx_no, type, item_id, warehouse_id, qty, delta_qty, ref_type, ref_id, ref_no, remark, created_by)
             SELECT ?, 'ADJUST', ?, ?, ?, ?, 'STOCKTAKE', ?, ?, ?, ?
             WHERE (SELECT changes()) > 0`).bind(txNo, itemId, warehouseId, Math.abs(diff), diff, st_id, String(st.st_no), '盘点调整', user.username));
            }
            if (!stmts.length)
                continue;
            const res = await env.DB.batch(stmts);
            for (let k = 0; k < res.length; k += 3) {
                if ((res[k + 2]?.meta?.changes || 0) > 0)
                    adjusted += 1;
            }
        }
        const done = await env.DB.prepare(`UPDATE stocktake SET status='APPLIED', applied_at=${sqlNowStored()} WHERE id=? AND status='APPLYING'`).bind(st_id).run();
        ...
        waitUntil(logAudit(env.DB, request, user, 'STOCKTAKE_APPLY', 'stocktake', st_id, {
            st_no: st.st_no,
            adjusted,
        }).catch(() => { }));
