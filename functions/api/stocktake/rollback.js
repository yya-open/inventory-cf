...
export const onRequestPost = async ({ env, request, waitUntil }) => {
    try {
        ...
        try {
            ...
            const rows = (await env.DB.prepare(`SELECT l.*, i.sku
           FROM stocktake_line l
           JOIN items i ON i.id=l.item_id
           WHERE l.stocktake_id=? AND l.diff_qty IS NOT NULL AND l.diff_qty != 0`).bind(st_id).all()).results || [];
            let reversed = 0;
            ...
            for (let i = 0; i < rows.length; i += CHUNK_LINES) {
                ...
                for (const l of part) {
                    ...
                    if (!itemId || !diff)
                        continue;
                    stmts.push(env.DB.prepare(`INSERT INTO stock_tx (tx_no, type, item_id, warehouse_id, qty, delta_qty, ref_type, ref_id, ref_no, remark, created_by)
               ...
               )`).bind(stocktakeRollbackTxNo(stNo, itemId), itemId, warehouseId, Math.abs(diff), -diff, st_id, stNo, `撤销盘点 ${stNo}`, user.username, st_id, itemId, warehouseId), env.DB.prepare(`INSERT INTO stock (item_id, warehouse_id, qty, updated_at)
               VALUES (?, ?, ?, ${sqlNowStored()})
               ON CONFLICT(item_id, warehouse_id) DO UPDATE SET qty=excluded.qty, updated_at=${sqlNowStored()}`).bind(itemId, warehouseId, Number(l.system_qty)));
                }
                if (!stmts.length)
                    continue;
                const res = await env.DB.batch(stmts);
                for (let k = 0; k < res.length; k += 2) {
                    if ((res[k]?.meta?.changes || 0) > 0)
                        reversed += 1;
                }
            }
            ...
            waitUntil(logAudit(env.DB, request, user, 'STOCKTAKE_ROLLBACK', 'stocktake', st_id, { st_no: stNo, reversed }).catch(() => { }));
