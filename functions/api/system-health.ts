...
export const onRequestGet: PagesFunction<{ DB: D1Database; JWT_SECRET: string }> = async ({ env, request }) => {
  try {
    ...
    const [pcAssets, latestState, counterRows, failedJobs, errors24h, lastRepair] = await Promise.all([
    ...
    const latestMissing = await env.DB.prepare(
      `SELECT COUNT(*) AS c FROM pc_assets a LEFT JOIN pc_asset_latest_state s ON s.asset_id=a.id WHERE s.asset_id IS NULL`
    ).first<any>().catch(() => ({ c: 0 }));
    return json(true, {
      schema,
      metrics: {
