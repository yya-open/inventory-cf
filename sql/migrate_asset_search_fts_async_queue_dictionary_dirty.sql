...
CREATE TRIGGER IF NOT EXISTS pc_assets_fts_ai AFTER INSERT ON pc_assets BEGIN
  INSERT OR REPLACE INTO pc_assets_fts(rowid, serial_no, brand, model, remark, disk_capacity, memory_size, search_text_norm)
...
CREATE TRIGGER IF NOT EXISTS monitor_assets_fts_ai AFTER INSERT ON monitor_assets BEGIN
  INSERT OR REPLACE INTO monitor_assets_fts(rowid, asset_code, sn, brand, model, size_inch, employee_no, employee_name, department, remark, search_text_norm)
...
CREATE TRIGGER IF NOT EXISTS audit_log_fts_ai AFTER INSERT ON audit_log BEGIN
  INSERT OR REPLACE INTO audit_log_fts(rowid, username, action, entity, entity_id, target_name, target_code, summary_text, search_text_norm)
