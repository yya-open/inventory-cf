<template>
  ...
  >
    <template #header>
      <div class="qr-header">
        <div class="qr-title">
          <span class="qr-title-main">扫码查看显示器信息</span><span class="qr-title-sub">可控长期码 · 信息实时更新</span>
        </div><el-tag
          v-if="row"
          size="small"
          type="info"
          effect="plain"
        >
          {{ [row.brand, row.model].filter(Boolean).join(' ') || '显示器' }}
        </el-tag>
      </div>
    </template><div
      v-loading="loading"
        ...
        <div class="qr-card">
          ...
          >
            <div class="qr-meta-line">
              <span class="k">资产编号</span><span class="v">{{ row.asset_code || '-' }}</span>
            </div><div class="qr-meta-line">
              <span class="k">SN</span><span class="v">{{ row.sn || '-' }}</span>
            </div><div class="qr-meta-line">
...
<script setup lang="ts">defineProps<{ visible:boolean; loading:boolean; row:Record<string,any>|null; link:string; dataUrl:string; isAdmin:boolean; canExport:boolean; canReset:boolean; statusText:(status:string)=>string }>(); const emit = defineEmits<{ 'update:visible':[boolean]; 'download-qr':[]; 'download-label':[]; 'open-link':[]; 'copy-link':[]; reset:[] }>();</script>
