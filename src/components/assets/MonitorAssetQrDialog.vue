...
<script setup lang="ts">defineProps<{ visible:boolean; loading:boolean; row:Record<string,any>|null; link:string; dataUrl:string; isAdmin:boolean; statusText:(status:string)=>string }>(); const emit = defineEmits<{ 'update:visible':[boolean]; download:[]; 'open-link':[]; 'copy-link':[]; reset:[] }>();</script>
