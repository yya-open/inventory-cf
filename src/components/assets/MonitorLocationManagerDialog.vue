...
<script setup lang="ts">defineProps<{ visible:boolean; rows:Array<Record<string,any>>; newName:string; parentId:string|number; locationParentOptions:Array<{value:number;label:string}>; isAdmin:boolean; buildLocLabel:(row:Record<string,any>)=>string }>(); const emit = defineEmits<{ 'update:visible':[boolean]; 'update:new-name':[string]; 'update:parent-id':[string|number]; create:[]; 'update-location':[Record<string,any>]; 'delete-location':[Record<string,any>] }>();</script>
