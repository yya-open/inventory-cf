...
<script setup lang="ts">defineProps<{ visible:boolean; loading:boolean; dataUrl:string; link:string; row:Record<string,any>|null; isAdmin:boolean; statusText:(status:string)=>string }>(); const emit = defineEmits<{ 'update:visible':[boolean]; 'download-qr':[]; 'download-label':[]; 'open-link':[]; 'copy-link':[]; 'reset-qr':[] }>();</script>
