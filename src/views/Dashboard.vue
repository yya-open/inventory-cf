      ...
      >
        ...
        <el-card shadow="never">
          <div style="color:#999;font-size:12px;">
            ...
            {{ data.summary.out_qty }}
          </div>
        </el-card>
        <el-card shadow="never">
          <div style="color:#999;font-size:12px;">
            {{ reportMode==='pc' ? '回收/归还数量' : '调整数量' }}
          </div>
          <div style="font-size:26px;font-weight:700;">
            {{ reportMode==='pc' ? (data.summary.recycle_qty ?? 0) : (data.summary.adjust_qty ?? 0) }}
          </div>
        </el-card>
        <el-card
          v-if="reportMode==='pc'"
      ...
      >
        <el-card shadow="never">
          ...
          <div style="display:flex; flex-direction:column; gap:6px;">
            <div
              v-for="r in seriesFilled"
              :key="r.day"
              style="display:flex; align-items:center; gap:10px;"
            >
              <div style="width:96px; color:#666; font-size:12px;">
                {{ r.day }}
              </div>
              <div style="flex:1; background:#f2f3f5; height:10px; border-radius:6px; overflow:hidden;">
                <div
                  :style="{ width: barWidth(r.qty) }"
                  style="height:10px; background:#409eff;"
                />
              </div>
              <div style="width:60px; text-align:right; color:#333; font-size:12px;">
                {{ r.qty }}
              </div>
            </div>
          </div>
